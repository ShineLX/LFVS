...
class Model(object):
    def __init__(self, is_training, config, name):
        ...
        self.copy_lf_batch_holder = tf.compat.v1.placeholder(tf.compat.v1.float32, shape = (None,H,W,config.channels), name = name + 'data_placeholder')
        ...
        self.grads, _ = tf.compat.v1.clip_by_global_norm(tf.compat.v1.gradients(self.loss, self.tvars, name = 'train_gradients'),
                                      self.config.gradient_clip, name = "clip_gradients_train")
    
        self.global_step=tf.compat.v1.train.get_or_create_global_step()      
        self.learning_rate = tf.compat.v1.train.exponential_decay(self.config.starter_learning_rate, self.global_step,
                                           self.config.decay_steps, self.config.decay_rate, staircase=False)
            
        self.optimizer = tf.compat.v1.train.GradientDescentOptimizer(self.learning_rate, name = "gradient_descent_train")
...
#        tf.compat.v1.train.export_meta_graph(dot +'/saver/models.meta')
        print ('graph_exported')


    def struct(self,copy_lf_batch_holder, is_training):
            
        cell = tf.compat.v1.nn.rnn_cell.LSTMCell(self.config.hidden_size, forget_bias=0.0, state_is_tuple=True,
                                                    reuse = not is_training)

        self.initial_state = cell.zero_state(self.config.batch_size, tf.compat.v1.float32)    
        
        tf_resize_tensor = tf.compat.v1.constant([self.config.patch_size, self.config.patch_size], name= self.name + 'interpolate_to_size')
        ...
        for col in range(1,self.config.lf_dim):
            ...
            for row in range(1,self.config.lf_dim):
                ...
                output = tf.compat.v1.identity(output, name="output_from_layers")
                ...
                target = tf.compat.v1.identity(target, name="target_original")
                self.loss = self.loss + tf.compat.v1.reduce_sum(tf.compat.v1.pow((target-output),2))
                
        self.output_stack = tf.compat.v1.stack(self.output_list)
    ...
    def prepare_input(self,row, col, lf_batch_holder, is_training):
        ...
        seq_input = tf.compat.v1.convert_to_tensor(seq_input, name = "sequence_input_tensor")
        seq_input = tf.compat.v1.reshape(seq_input, (-1,self.config.patch_size,self.config.patch_size,3*self.config.channels))
    ...
    def layers(self,row, col, seq_input, cell, state, tf_resize_tensor, is_training):
        ...
        with tf.compat.v1.variable_scope('encoder',reuse=tf.compat.v1.AUTO_REUSE):
            ...
            with tf.compat.v1.variable_scope('conv_3',reuse=tf.compat.v1.AUTO_REUSE):
            ...
            lstm_input = tf.compat.v1.reshape(batchnorm_conv3_b, [-1,4*4*64])
            lstm_input = tf.compat.v1.identity(lstm_input, name="lstm_input")
        ...
        with tf.compat.v1.variable_scope('decoder', reuse = tf.compat.v1.AUTO_REUSE):
            decoder_input = tf.compat.v1.reshape(cell_output,[-1,4,4,64])
            decoder_input = tf.compat.v1.identity(decoder_input,name = "decoder_input")
                
            with tf.compat.v1.variable_scope('bicubic_interpolate',reuse=tf.compat.v1.AUTO_REUSE):
                out_bicubic = tf.compat.v1.image.resize_bicubic(decoder_input,tf_resize_tensor,align_corners=False,name='op_bicubic_interp')
                    
            with tf.compat.v1.variable_scope('dec_conv1_',reuse=tf.compat.v1.AUTO_REUSE):
            ...
            with tf.compat.v1.variable_scope('dec_conv2_',reuse=tf.compat.v1.AUTO_REUSE):
            ...
            with tf.compat.v1.variable_scope('dec_conv3_',reuse=tf.compat.v1.AUTO_REUSE):
        ...
        with tf.compat.v1.variable_scope('out_level',reuse=tf.compat.v1.AUTO_REUSE):
            out_conv = tf.compat.v1.nn.relu(tf.compat.v1.nn.conv2d(
                            dec_batchnorm_conv3_b, self.variable_get([3,3,3,1], 'out_conv_filter'),strides = [1,1,1,1], 
                            padding = 'SAME'))
            output = tf.compat.v1.nn.bias_add(out_conv, self.variable_get([1],'out_conv_bias'))         
            
            return output, state

    def variable_get(self,shape, name):
        return tf.compat.v1.get_variable(name, shape, tf.compat.v1.float32)
        

    def conv2D_batchnorm(self,input_, kernel_shape, kernel_name,strides, padding,bias_name,batch_norm_name, is_training):
        conv = tf.compat.v1.nn.relu(tf.compat.v1.nn.conv2d(input_, self.variable_get(kernel_shape, kernel_name),
                                                                            strides = strides, padding = padding,data_format='NHWC'))          
        conv_b = tf.compat.v1.nn.bias_add(conv, self.variable_get([kernel_shape[-1]],name = bias_name))
        batchnorm_conv_b = tf.compat.v1.keras.layers.BatchNormalization(axis = -1)(conv_b, training = is_training) 
...
class Input(object):
  ...
  def __init__(self, config, name=None):  
    ...
    self.tf_lf_placeholder = tf.compat.v1.placeholder(tf.compat.v1.float32, shape = (None,H,W,config.channels), name = name + '_placeholder')
    self.tf_lf_data = tf.compat.v1.data.Dataset.from_tensor_slices(self.tf_lf_placeholder)
...
with g.as_default():
        saver_hook = tf.compat.v1.train.CheckpointSaverHook(checkpoint_dir = train_config +'checkpoint_saver', save_steps = 10)
        summary_hook = tf.compat.v1.train.SummarySaverHook(output_dir = train_config + 'summary',save_steps =2,scaffold=tf.compat.v1.train.Scaffold())    
        config = tf.compat.v1.ConfigProto(allow_soft_placement=True, log_device_placement=True)
        
        print ('**************** entering in session ****************')
        
        with tf.compat.v1.train.MonitoredTrainingSession(config = config,hooks = [saver_hook],
                                           checkpoint_dir = train_config + 'checkpoint_saver') as sess:
