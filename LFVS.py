...
flags.DEFINE_integer("gradient_clip", 5,
                     "gradient clip")
flags.DEFINE_integer("strides", 64,
                     "strides")
flags.DEFINE_integer("save_steps", 5,
                     "strides")
flags.DEFINE_integer("save_output_steps", 10,
                     "save test output in every number of these steps")
flags.DEFINE_string("model", "model_5",
                     "model_info")
...
hidden_dims = [int(FLAGS.patch/16),int(FLAGS.patch/16),64]
...
class TrainConfigSmall(object):
  ...
  if (FLAGS.dot == "."):
      ...
      base_dir =  FLAGS.dot + "/tmp/data/training/"    
...
class TestConfig(object):
  ...
  if (FLAGS.dot == "."):
      ...
      base_dir =  FLAGS.dot + "/tmp/data/test/"
  save_path = FLAGS.dot + FLAGS.save_path + str(FLAGS.model) + "/"
  num_files = FLAGS.num_files
  train_single_image = True
  allowed_lf_dim = FLAGS.allowed_lf_dim
...
lf_train_data = np.load(FLAGS.dot + FLAGS.save_path + "train_data/lf_train_data.npy") 
print ('lf_train_data', np.shape(lf_train_data))

lf_test_data = np.load(FLAGS.dot + FLAGS.save_path + "test_data/lf_test_data.npy")
...
class Model(object):
    ...
    from _layers import layers_new_5
...
#        self.loss_summary = 
#        tf.summary.scalar(name+"_loss_summary", self.loss)
#        self.output_summary = tf.summary.image(name+"_output", tf.squeeze(self.output_stack))
...
#                output, state_ = self.layers(row, col, seq_input, cell, state, tf_resize_tensor, is_training)
                output, state_ = self.layers_new_5(row, col, seq_input, cell, state, tf_resize_tensor, is_training)
                ...
                if (self.config.is_training):
                    print ("inside trainig summary")
                    self.training_loss_summary = tf.summary.scalar("train_loss_summary", self.loss)
#                else:
#                    tf.summary.scalar("test_loss_summary", self.loss)
...
for lr in lr_list:     
    ...
    with g.as_default():
        ...
        merge_summ = tf.summary.merge([m_train.training_loss_summary])
...
#        summary_hook = tf.train.SummasrySaverHook(output_dir = train_config.save_path + lr_path + 'summary',save_steps =2,scaffold=tf.train.Scaffold())    
        ...
        with tf.train.MonitoredTrainingSession(config = sess_config,hooks = [saver_hook],
                                           checkpoint_dir = train_config.save_path + lr_path) as sess:
            ...
            while not sess.should_stop():
                ...
                summary = sess.run(merge_summ, feed_dict = {m_train.copy_lf_batch_holder : train_batch})
                
                train_writer.add_summary(summary,g_step_no)
                train_writer.flush()
...
flags.DEFINE_integer("strides", 8,
                     "strides")
flags.DEFINE_integer("save_steps", 50,
                     "strides")
flags.DEFINE_integer("save_output_steps", 250,
                     "save test output in every number of these steps")
flags.DEFINE_string("model", "model_3",
                     "model_info")
flags.DEFINE_list("lr_list", [0.1,0.01,0.001,0.0001,0.00001],
                     "list of learning rates")
flags.DEFINE_float("dropout_rate", 1.0,
