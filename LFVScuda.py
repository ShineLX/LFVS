...
# -*- coding: utf-8 -*-
"""
Created on Fri Jul 12 18:12:34 2019
...
#        tf.train.export_meta_graph(dot +'/saver/models.meta')
    ...
    def struct(self,copy_lf_batch_holder, is_training):
        ...
        for col in range(1,self.config.lf_dim):
            ...
            for row in range(1,self.config.lf_dim):
                ...
                output, state_, cuda_state_ = self.layers(row, col, seq_input, cell,self.cuda_cell,cuda_state, state, tf_resize_tensor, is_training)
                state = state_
    ...
    def layers(self,row, col, seq_input, cell, cuda_cell,cuda_state,state, tf_resize_tensor, is_training):
        ...
        with tf.variable_scope('encoder',reuse=tf.AUTO_REUSE):
            ...
            lstm_input = tf.reshape(lstm_input, [1,-1,4*4*64])
...
#            (cell_output, state) = cell(lstm_input, state)
        ...
        cell_output = self.cuda_outputs
                
        with tf.variable_scope('decoder', reuse = tf.AUTO_REUSE):
            cell_output = tf.reshape(cell_output,[-1,4*4*64])
        ...
        with tf.variable_scope('out_level',reuse=tf.AUTO_REUSE):
            ...
            return output, state, self.cuda_output_states
