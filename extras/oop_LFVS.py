...
flags.DEFINE_string("data_path", base_dir,
...
lr_path = dot + '/saver/lr.npy'
loss_path = dot + '/saver/loss.npy'
lr_list, loss_list = generate_lf_dataset.get_lr_loss_list(lr_path, loss_path)

lf_dim, lf_dataset_new = generate_lf_dataset.get_data(base_dir,patch_size, strides)
...
with g.as_default():
        saver_hook = tf.train.CheckpointSaverHook(checkpoint_dir = dot +'/saver/checkpoint_saver', save_steps = 1)
        ...
        with tf.train.MonitoredTrainingSession(config = config,hooks = [saver_hook],
            ...
            while not sess.should_stop():
                ...
                np.save(dot +'/saver/loss.npy', np.array(m_train.loss_list))   
            
                np.save(dot + '/saver/lr.npy', np.array(m_train.lr_list))            
