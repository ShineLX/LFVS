...
def read_lightfield(data_folder):
    ...
    #taking only one channel right now
    light_field = np.zeros((params["num_cams_x"], params["num_cams_y"], params["height"], params["width"], 1), dtype=np.uint8)
    ...
    for idx, view in enumerate(views):
        ...
        try:
            img = read_img(fpath)
            ## added by rohit to take only one channel
            img =np.reshape((cv2.cvtColor(img, cv2.COLOR_BGR2YCR_CB))[:,:,0],(params["height"],params["width"],1))
